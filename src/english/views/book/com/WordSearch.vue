...
// 查询表单
const queryForm = ref<VocabularyQuery>({
  status: null,
  english: null,
  chinese: null,
});
...
const initCategory = async () => {
  const res = await vocabularyStore.fetchStatusDict();
  if (res.code === -1) {
...
const debouncedFetch = debounce(() => {
  // 更新分页查询参数
  vocabularyStore.pageQuery.query = { ...queryForm.value };
  // 重置页码为1
  vocabularyStore.pageQuery.pageNum = 1;
  // 执行查询
  vocabularyStore.fetchVocabularyPage();
}, 300);
...
onMounted(async () => {
  await initCategory();
  // 初始化时同步查询条件
  queryForm.value = { ...vocabularyStore.pageQuery.query };
  // 执行查询
...
watch(
    () => vocabularyStore.pageQuery.query,
    (newVal) => {
